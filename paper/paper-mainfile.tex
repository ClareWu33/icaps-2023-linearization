...
\input{03--linearization-ground.tex}
 ...
 \begin{figure}[h]
	...
	\scalebox{0.75}{
		\begin{subfigure}{7cm} 
			...
			\action{Prec}{SinglePrec,
				width  = 2.5em, 
				height = 2.5em, 
				pre length = 1em,
				eff length = 1em,
				body = {left = 4em of Del} 
			};  		
			
			\action{Add}{SingleAdd,
				width  = 2.5em, 
				height = 2.5em, 
				pre length = 1em,
				eff length = 1em,
				body = {left = 4em of Prec} 
		};	
					  
			
			\path[every node/.style={font=\sffamily\small}]
			(Prec) edge [->,bend right] node [left]  {} (Del);
			\end{tikzpicture}
	...
	\scalebox{0.75}{
		\begin{subfigure}{7cm} 
			...
			\action{Del}{SingleDelete,
				...
				pre length = 1em,
				eff length = 1em,
				body = {left = 4em of Add} 
			};  			 
			
			\path[every node/.style={font=\sffamily\small}]
			(Del) edge [->, bend right] node [left]  {} (Add);
			\end{tikzpicture}
...
We test the $panda_{\pi}$ solver by \cite{useClassicalHuristicICAPS18,useClassicalHeuristicIJCAI19,progressionsearchJAIR20} on both the original and the linearized problems. According to \cite{holler2021loop} the best setting for solving partial-order problems is \textit{GA* FF th+gi}, and the best setting for total-order is \textit{GBFS Add th+s}. 
Both turn the problem into a classical planning problem (in RC model). The former uses weighted A* search \todo{confirm weight}, the FF heuristic by \cite{FF}, and visited lists.  The latter uses Greedy best first search, Add heuristic by \cite{Add}, and visited lists.

Hashing is used to speed up checks to the visited list. Both approaches use \textit{th}, i.e. uses task hash approximation (computes  a  hash  value  that  only  incorporates  information  on which task is how often in a given TN) to replace tests for task network isomorphism. 
For \textit{th+s} first hash with taskhash and then use the exact test on all TNs in the hash bucket.
When \textit{+gi} follows, the exact test is performed in case of a hash collision. 


%incomplete: (GA* FF th)
%/pandaPIengine -H "rc2(ff)" -w 2  --noLayers --noGIcheck (GA* FF oh)
%/pandaPIengine -H "rc2(ff)" -w 2  --noLayers  (GA* FF oh+gi)

%incomplete: ./pandaPIengine -H "rc2(add)" -g "none" -Q  --noLayers   --noOrderPairs   --noGIcheck  (GBFS Add th)
%./pandaPIengine -H "rc2(add)" -g "none"  -T   --noLayers   --noOrderPairs   --noGIcheck  (GBFS Add s)
%./pandaPIengine -H "rc2(add)" -g "none"  --noLayers   --noOrderPairs  --noGIcheck  (GBFS Add th+s)
